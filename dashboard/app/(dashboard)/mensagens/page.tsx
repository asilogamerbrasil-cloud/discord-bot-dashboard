...
interface MensagemProgramada {
  id: number;
  nome: string;
  tipo: 'manual' | 'shopee_preset';
  mensagem: string;
  timerIntervalo: number;
  servidoresCanais: string | null;
  shopeePreset: string | null;
...
const TIMER_PRESETS = [
  { label: '30 min', valor: 1800 },
  { label: '1 hora', valor: 3600 },
  { label: '2 horas', valor: 7200 },
  { label: '4 horas', valor: 14400 },
  { label: '6 horas', valor: 21600 },
  { label: '12 horas', valor: 43200 },
  { label: '24 horas', valor: 86400 },
  { label: 'Personalizado', valor: -1 },
];

const SHOPEE_PRESETS_LIST = [
  { key: 'games_promo', nome: 'Computadores Gamer em Promocao', desc: 'Notebooks e PCs gamer com desconto' },
  { key: 'games_vendidos', nome: 'Computadores Mais Vendidos do Mes', desc: 'PCs e notebooks mais populares' },
  { key: 'hardware', nome: 'Hardware em Alta', desc: 'Placas de video, memorias, SSDs, processadores' },
  { key: 'perifericos', nome: 'Perifericos em Alta', desc: 'Mouses, headsets, teclados, monitores gamer' },
  { key: 'geral', nome: 'Tecnologia & Games (Geral)', desc: 'Mix de todas as categorias, produtos variados' },
];
...
export default function PaginaMensagens() {
  ...
  async function carregarMensagens() {
    try { const r = await fetch('/api/mensagens'); if (r.ok) setMensagens(await r.json()); }
  ...
  function adicionarCanal() {
    ...
    const jaExiste = canaisSelecionados.find(c => c.canalId === canalAtual && c.servidorId === servidorAtual);
    if (jaExiste) return;

    setCanaisSelecionados([...canaisSelecionados, {
      servidorId: servidorAtual,
  ...
  function removerCanal(idx: number) {
    setCanaisSelecionados(canaisSelecionados.filter((_, i) => i !== idx));
  }
  ...
  function inserirFormatacao(wrap: string, suffix?: string) {
    ...
    const antes = (editando.mensagem || '').substring(0, start);
    const depois = (editando.mensagem || '').substring(end);
    const novoTexto = `${antes}${wrap}${selected}${suffix || wrap}${depois}`;
    setEditando({ ...editando, mensagem: novoTexto });
    setTimeout(() => {
      textarea.focus();
  ...
  async function salvar() {
    ...
    const body: Record<string, unknown> = {
      nome: editando.nome || 'Nova Mensagem',
      tipo: editando.tipo || 'manual',
      mensagem: editando.mensagem || '',
      timerIntervalo: getTimerValor(),
      servidoresCanais: canaisSelecionados,
      shopeePreset: editando.tipo === 'shopee_preset' ? (editando.shopeePreset || 'geral') : null,
    };
    ...
    try {
      ...
      const r = await fetch('/api/mensagens', {
        ...
        body: JSON.stringify(editando.id ? { ...body, id: editando.id } : body),
      });
      if (r.ok) {
        await carregarMensagens();
        setEditando(null);
        setStatusMsg({ tipo: 'sucesso', texto: 'Salvo!' });
      } else {
        const d = await r.json();
        setStatusMsg({ tipo: 'erro', texto: d.erro || 'Erro' });
      }
    } catch {
      setStatusMsg({ tipo: 'erro', texto: 'Erro ao salvar' });
    }
  }
    ...
    <div className="space-y-6">
      ...
      {mensagens.length === 0 && !editando && (
        <div className="bg-[#2B2D31] border border-[#1E1F22] rounded-lg p-12 text-center">
          ...
          <p className="text-[#B5BAC1] text-sm mb-4">Crie mensagens para serem publicadas automaticamente nos servidores.</p>
          ...
          <div className="bg-[#2B2D31] border border-[#313338] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    { key: 'shopee_preset' as const, label: 'Shopee Afiliados', icon: ShoppingBag, desc: 'Publica promocoes automaticas da Shopee' },
                  ].map(op => (
                    <button key={op.key} type="button" onClick={() => { setEditando({ ...editando, tipo: op.key, shopeePreset: op.key === 'shopee_preset' ? 'geral' : null }); }}
              ...
              {editando.tipo === 'shopee_preset' && (
                <div>
                  <label className="block text-xs text-[#B5BAC1] mb-2">Preset da Shopee</label>
                  <div className="space-y-2">
                    {SHOPEE_PRESETS_LIST.map(p => (
                      <label key={p.key} onClick={() => setEditando({ ...editando, shopeePreset: p.key })}
                        className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${editando.shopeePreset === p.key ? 'border-[#EE4D2D] bg-[#EE4D2D]/10' : 'border-[#1E1F22] hover:border-[#3F4147]'}`}>
                        <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${editando.shopeePreset === p.key ? 'border-[#EE4D2D]' : 'border-[#3F4147]'}`}>
                          {editando.shopeePreset === p.key && <div className="w-2 h-2 rounded-full bg-[#EE4D2D]"/>}
                        </div>
                        <div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-2 bg-[#1E1F22] rounded-t-lg p-2 border border-b-0 border-[#313338]">
                    ...
                    <button type="button" onClick={() => inserirFormatacao('```\n', '\n```')}
                      className="p-1.5 rounded hover:bg-[#313338] text-[#B5BAC1] hover:text-white transition-colors" title="Bloco de Codigo">
                      <List className="w-4 h-4"/>
                    </button>
                    <button type="button" onClick={() => inserirFormatacao('# ', '')}
                      className="p-1.5 rounded hover:bg-[#313338] text-[#B5BAC1] hover:text-white transition-colors" title="Titulo">
                      <Type className="w-4 h-4"/>
                    </button>
                    <button type="button" onClick={() => {
                      if (editando) setEditando({ ...editando, mensagem: (editando.mensagem || '') + '\n> ' });
                    }} className="p-1.5 rounded hover:bg-[#313338] text-[#B5BAC1] hover:text-white transition-colors" title="Bloco de Citacao">
                      <Copy className="w-4 h-4"/>
                    </button>
                  </div>
                  <textarea id="editor-mensagem" value={editando.mensagem || ''}
                    onChange={e => setEditando({ ...editando, mensagem: e.target.value })}
                    rows={8} placeholder="Escreva sua mensagem aqui... Use **negrito**, *italico*, __sublinhado__, ||spoiler||, `codigo`, [links](url) etc."
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <select value={servidorAtual} onChange={e => carregarCanais(e.target.value)}
                    className="flex-1 bg-[#1E1F22] border border-[#3F4147] rounded-md px-3 py-2 text-white text-sm focus:border-[#5865F2] focus:outline-none">
                    <option value="">Selecione servidor...</option>
                    {servidores.map(s => <option key={s.id} value={s.id}>{s.nome}</option>)}
                  </select>
                  {canaisServidor.length > 0 && (
                    <>
                      <select value={canalAtual} onChange={e => setCanalAtual(e.target.value)}
                        className="flex-1 bg-[#1E1F22] border border-[#3F4147] rounded-md px-3 py-2 text-white text-sm focus:border-[#5865F2] focus:outline-none">
                        <option value="">Selecione canal...</option>
                        {canaisServidor.map(c => <option key={c.id} value={c.id}># {c.nome}</option>)}
                      </select>
                      <button type="button" onClick={adicionarCanal} disabled={!servidorAtual || !canalAtual}
                        className="bg-[#5865F2] hover:bg-[#4752C4] disabled:opacity-40 text-white px-3 py-2 rounded-md text-sm flex items-center gap-1">
                ...
                {canaisSelecionados.length > 0 ? (
                  ...
                  <p className="text-xs text-[#72767D] py-2">Adicione pelo menos um servidor e canal para enviar as mensagens.</p>
                )}
              </div>

              <div className="flex gap-2 pt-2 border-t border-[#1E1F22]">
                <button type="button" onClick={salvar} disabled={canaisSelecionados.length === 0}
                  className="bg-[#5865F2] hover:bg-[#4752C4] disabled:opacity-40 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2">
                ...
                {editando.id && (
                  <button type="button" onClick={() => excluir(editando.id!)} className="text-red-400 hover:text-red-300 px-4 py-2 rounded-md text-sm ml-auto">Excluir</button>
                )}
              </div>
            </div>
          </div>
